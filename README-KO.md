...
Learn Harness Engineering은 AI 코딩 에이전트 공학에 전념하는 강의입니다. 업계에서 가장 앞선 해네스 엔지니어링(Harness Engineering) 이론과 실천을 깊이 연구하여 종합했습니다. 핵심 참고 자료는 다음과 같습니다.
...
> **빠르게 시작하려면?** [`skills/harness-creator/`](./skills/) 스킬(skill)을 사용하면 몇 분 만에 프로덕션 수준의 해네스(AGENTS.md, 기능 목록, init.sh, 검증 워크플로우)를 직접 프로젝트에 스캐폴딩(scaffold)할 수 있습니다.
...
이것은 모델 문제가 아닙니다. 해네스 문제입니다.

증거는 명확합니다. Anthropic이 통제된 실험을 수행했습니다. 동일한 모델(Opus 4.5), 동일한 프롬프트("2D 레트로 게임 에디터 만들기"). 해네스 없이는 20분 동안 $9를 사용했으나 동작하지 않는 결과물이 나왔습니다. 완전한 해네스(플래너 + 생성기 + 평가기)를 사용했을 때는 6시간 동안 $200을 사용해 실제로 플레이할 수 있는 게임을 만들었습니다. 모델은 바뀌지 않았습니다. 해네스가 바뀌었습니다.
...
## 해네스 엔지니어링이란 실제로 무엇인가

해네스 엔지니어링(Harness Engineering)은 모델이 안정적인 결과를 낼 수 있도록 모델 주변에 완전한 작업 환경을 구축하는 것입니다. 더 나은 프롬프트를 작성하는 것이 아닙니다. 모델이 운영되는 시스템을 설계하는 것입니다.

해네스에는 다섯 가지 하위 시스템이 있습니다.
...
1. **강의(Lectures)**: 해네스 엔지니어링의 이론을 설명하는 12개 개념 단원.
...
이 프로젝트는 강력한 실용적 가치, 충분한 실제 제품 복잡성, 그리고 해네스 개선 전후를 관찰하기에 적합한 환경을 갖추고 있어 선택되었습니다.

각 강의 프로젝트의 시작점/풀이는 해당 진화 단계의 이 Electron 앱의 완전한 복사본입니다. P(N+1)의 시작점은 P(N)의 풀이에서 파생됩니다 — 앱은 해네스 기술이 성장함에 따라 함께 진화합니다.
...
| [L02](./docs/lectures/lecture-02-what-a-harness-actually-is/index.md) | "해네스"는 실제로 무엇을 의미하는가? | 다섯 가지 하위 시스템: 지시사항, 상태, 검증, 범위, 라이프사이클 |
...
| [L08](./docs/lectures/lecture-08-why-feature-lists-are-harness-primitives/index.md) | 기능 목록이 왜 해네스의 기본 구성 요소인가? | 에이전트가 무시할 수 없는 기계 가독형 범위 경계 |
...
| [L11](./docs/lectures/lecture-11-why-observability-belongs-inside-the-harness/index.md) | 가관측성(observability)이 왜 해네스 내부에 있어야 하는가? | 에이전트가 무엇을 했는지 볼 수 없으면 무엇이 잘못되었는지 고칠 수 없다 |
...
| 프로젝트 | 수행 내용 | 해네스 메커니즘 |
|---------|----------|----------------|
| [P01](./docs/projects/project-01-baseline-vs-minimal-harness/index.md) | 동일한 작업을 두 번 실행: 프롬프트만 사용 vs. 규칙 우선 | 최소 해네스: AGENTS.md + init.sh + feature_list.json |
...
| [P06](./docs/projects/project-06-runtime-observability-and-debugging/index.md) | 처음부터 완전한 해네스 구축 (캡스톤 프로젝트) | 완전한 해네스: 모든 메커니즘 + 가관측성 + 절제 연구 |
...
- 해네스 설계에 대한 체계적인 이해를 원하는 연구자 또는 빌더
...
- 모든 프로젝트는 약한 해네스 대 강한 해네스 결과를 비교합니다
