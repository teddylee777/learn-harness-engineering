...
- **첫 등장 형식**: 본문에서 처음 나오는 전문 용어는 `한국어(English)` 형태로 병기합니다. 예: `해네스(harness)`, `에이전트(agent)`.
...
| English | 한국어 | 정의 |
|---------|--------|------|
| Harness | 해네스 | AI 코딩 에이전트가 안정적으로 동작하도록 환경·상태·검증·제어를 묶어서 제공하는 외골격(스캐폴딩) 시스템. 본 강의의 중심 개념. |
| Harness Engineering | 해네스 엔지니어링 | 해네스를 설계·구축·유지보수하는 공학적 실천 분야. |
...
| English | 한국어 | 정의 |
|---------|--------|------|
| Initialization | 초기화 | 새 프로젝트나 작업 환경에서 해네스를 설정하는 단계. |
