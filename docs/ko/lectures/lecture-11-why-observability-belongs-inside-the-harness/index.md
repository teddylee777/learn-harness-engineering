...
> 실습 프로젝트: [Project 06. 완전한 해네스 (캡스톤)](./../../projects/project-06-runtime-observability-and-debugging/index.md)
...
이것은 에이전트의 능력 부족 문제가 아닙니다. 해네스가 충분한 관측 가능성(observability)을 제공하지 않는 것입니다. **관측 가능성 없이는 에이전트가 불확실성 속에서 결정을 내리고, 평가는 주관적 판단이 되며, 재시도는 맹목적인 방황이 됩니다.** OpenAI와 Anthropic 모두 신뢰성을 증거 문제로 정의합니다—해네스는 런타임 동작과 평가 신호를 다음 결정을 안내할 수 있는 형태로 노출해야 합니다.
...
- **프로세스 관측 가능성(Process observability)**: 계획, 채점 루브릭(rubric), 인수 기준 등 해네스 결정 산출물에 대한 가시성입니다. "이 변경이 왜 수용되어야 하는가"에 대해 답합니다.
...
해네스에 관측 가능성이 없으면 네 가지 유형의 문제가 체계적으로 나타납니다.
...
"계획자-생성자-평가자(planner-generator-evaluator)" 세 가지 역할 워크플로를 사용하는 해네스가 "앱에 다크 모드 추가" 작업을 실행하는 것을 상상해 보세요.
...
3. 프로세스 관측 가능성은 로그로 해결할 수 없습니다—스프린트 계약과 채점 루브릭은 해네스 수준의 지원이 필요한 구조화된 산출물입니다.

## 올바르게 하는 방법

### 1. 런타임 신호 수집을 해네스에 구축하라

에이전트가 자체 로그를 출력하는 것에 의존하지 마세요. 해네스가 자동으로 이러한 신호를 수집해야 합니다.
...
각 해네스 세션에 대한 트레이스(trace)를, 각 작업에 대한 스팬(span)을, 각 검증 단계에 대한 하위 스팬을 생성하세요. 표준 속성을 사용해 주요 정보에 주석을 달 수 있습니다. 이렇게 하면 관측 가능성 데이터가 표준 툴체인(Jaeger, Zipkin)과 통합됩니다.

## 실제 사례

계획자-생성자-평가자 워크플로를 사용하는 해네스가 "다크 모드 지원 추가"를 실행합니다.
...
- **관측 가능성은 해네스 아키텍처의 속성입니다**—나중에 추가되는 기능이 아니라, 설계 중에 고려되어야 하는 핵심 역량입니다.
...
1. **관측 가능성 격차 분석**: 현재 해네스에서 시스템 계층과 프로세스 계층의 관측 가능성을 감사합니다. 기존 신호로 구별할 수 없는 시스템 상태를 찾아 추가 사항을 제안하세요.
